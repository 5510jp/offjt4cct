...
「関数の自動補完」だけで使うのはもったいない！  
AIは **「要件定義書」を渡して、アプリの雛形を丸ごと作ってもらう** 時にこそ、その真価を発揮します。

今回は、社内イベントやランチ決めで使えそうな **「リアルタイム投票アプリ」** を題材に、  
AIにどのような指示（プロンプト）を出せば意図通りのコードが返ってくるのか、その実験結果を共有します。
...
まず、空のフォルダをVS Codeで開きます。  
次に、Copilot Edits（またはチャット欄）を開き、以下のプロンプト（指示）を投げます。
