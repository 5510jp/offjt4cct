...
現在参画しているプロジェクトではドキュメントを **MarkDown形式** で作成して、Github上でソースコードと一緒に管理しています。

そうなると、図（フローやシーケンスなど）についてもテキストで管理したくなります。

**Mermaid記法**は、こうしたMarkdownベースのドキュメント管理と非常に相性が良い作図手法です。

この記事では、テキスト（文字）だけで簡単に、きれいな図を描くことができる「Mermaid（マーメイド）記法」を紹介します。

期待できるメリットは以下の通りです。
* ドキュメントとの一元管理:  
    仕様書（テキスト）と図（テキスト）を同じファイルで管理・修正できます。
* 作業効率の向上:  
    テキストを記述するだけで図が自動生成され、修正も容易です。
* バージョン管理の容易さ:  
    Gitなどで仕様書と図の変更履歴（差分）をまとめて明確に管理できます。
